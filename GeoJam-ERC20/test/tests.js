...
describe('Geojam (JAM) With Protection', function() {
  ...
  const TOTAL_SUPPLY = 8000000000n * EXT;
  const PROTECTION_FEE = 8420000n * EXT;
  ...
  const ProtectionConfig = {
    ...
    LiquidityAmountTrap_amount: 1110000n * EXT,
    ...
    TokensToPutIntoLiquidityPool: 30000000n * EXT,


    PredeployedLPS: '0x80A703aD8f590035f2A18BD433a847694c83c069',
  ...
  before('snapshot', async function() {
    ...
    const JAMToken = await ethers.getContractFactory('JamEth');
    ext = await JAMToken.deploy(owner);
    await ext.deployed();
  ...
  describe("Liquidity Protection Service", () => {
    ...
    it('Should disable protection', async function() {
      ...
      await expect(ext.connect(user1Signer).disableProtection())
        .to.be.reverted;
      await setTimeAndMine(1642291199)
